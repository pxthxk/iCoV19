...
if cohort:
    cohortDir = outputDir + cohort.replace(" ", "-") + "/"
else:
...
samples = [i.split(".txt")[0] for i in os.listdir(cohortDir) if i.endswith(".txt")]

for sample in samples:
    if sample in list(dfStats["Sample"].astype(str)):
        rnaEdits = pd.read_csv(cohortDir + sample + ".txt", sep="\t")
...
cummFreq["Count (>= " + str(cutoffFreq) + ")"] = (cummFreq.iloc[:,3:len(cummFreq.columns)-2] >= cutoffFreq).sum(axis=1)
...
cummFreq.to_csv(cohortDir + "Frequencies.tsv", index=False, sep="\t")
...
cummCount.to_csv(cohortDir + "Counts.tsv", index=False, sep="\t")
